...
public class S3PrivateFileService {
  ...
  //Pre-Signed URL 생성
  private String generatePresignedUrl(String fileName, int expireTime) {
    GetObjectRequest req = GetObjectRequest.builder()
        .bucket(bucketName)
        .key(fileName)
        .build();
    GetObjectPresignRequest objReq = GetObjectPresignRequest.builder()
        .signatureDuration(Duration.ofMinutes(expireTime))
        .getObjectRequest(req)
        .build();

    PresignedGetObjectRequest request = s3Presigner.presignGetObject(objReq);
    log.info("Pre-signed URL : {}", request.url());
  ...
  public void deleteFiles(List<String> imgUrls) {
    List<String> fileNames = imgUrls.stream()
        .map(S3PrivateFileService::extractFileNameFromUrl)
        .toList();
